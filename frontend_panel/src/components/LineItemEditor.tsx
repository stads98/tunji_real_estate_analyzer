...
export function LineItemEditor({ lineItems, onChange, totalSqft }: LineItemEditorProps) {
    ...
    <Card className="border-2 border-primary/20">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Edit costs and descriptions. Add custom items. Ready for lender submission.
              </p>
            </div>
          </div>
      ...
      <CardContent className="p-6">
        ...
        <div className="space-y-6">
          {(['structural', 'systems', 'interior', 'exterior'] as const).map(category => {
                  ...
                  {items.map((item, idx) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Select
                          value={item.category}
                          onValueChange={(value: any) => updateLineItem(item.id, { category: value })}
                        >
          ...
          {lineItems.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p>No line items yet. Fill out property condition details above to generate items,</p>
        ...
        {lineItems.length > 0 && (
          <div className="mt-6 pt-6 border-t">
            <div className="flex items-center justify-between p-4 rounded-lg bg-primary/5 border-2 border-primary/20">
              <div>
                <p className="text-sm text-muted-foreground">Total Estimated Rehab Cost</p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {lineItems.length} line item{lineItems.length !== 1 ? 's' : ''}
                  {totalSqft && ` • ${formatCurrency(totalCost / totalSqft)}/sqft`}
                </p>
              </div>
              ...
              {(['structural', 'systems', 'interior', 'exterior'] as const).map(category => {
                ...
                return (
                  <div key={category} className="p-3 rounded-lg border bg-card">
                    <Badge className={`${categoryColors[category]} mb-2`} variant="outline">
                      {categoryLabels[category]}
                    </Badge>
                    <p className="font-bold">{formatCurrency(categoryTotal)}</p>
